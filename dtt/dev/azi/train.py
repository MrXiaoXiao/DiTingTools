from src.tfDataset.BAZDataset import get_Azi_training_dataset 
from src.models.DiTingBAZ_Basic import DiTingBAZ_Basic
...
def train(cfgs=None):
    ...
    hist = model.fit(training_dataset.repeat().batch(cfgs['Training']['batchsize']).prefetch(tf.data.experimental.AUTOTUNE),
                        ...
                        validation_data=validation_data_gen.repeat().batch(cfgs['Training']['batchsize']).prefetch(tf.data.experimental.AUTOTUNE),
