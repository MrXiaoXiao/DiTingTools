...
####################################################
def get_instance_for_BAZReg_training(dataset_name = 'DiTing',
                                        dataset_path = None,
                                        part = None,
                                        key = None,
                                        P = None,
    ...
    if norm_factor == 0:
        ...
        temp_data_X[:,:] /= norm_factor
    
    rescale_factor = np.random.uniform(low=0.5,high=1.5)

    temp_data_X[:,:] *= rescale_factor

    temp_data_Y[0] = np.sin(np.deg2rad(baz))
    temp_data_Y[1] = np.cos(np.deg2rad(baz))
...
class DiTingAziGenerator:
    ...
    def __init__(self, csv_hdf5_mapping_dict):
        ...
        if self.name == 'DiTing':
            self.csv_file =  self.csv_file[pd.to_numeric(self.csv_file['baz'], errors='coerce').notnull()]
        elif self.name == 'STEAD':
            self.csv_file =  self.csv_file[pd.to_numeric(self.csv_file['back_azimuth_deg'], errors='coerce').notnull()]
        elif self.name == 'INSTANCE':
            self.csv_file =  self.csv_file[pd.to_numeric(self.csv_file['path_backazimuth_deg'], errors='coerce').notnull()]
        else:
            pass
        ...
        while True:
            ...
            for idx in range(0,len(indexes)):
                if self.name == 'DiTing':
                    ...
                    baz = azi2baz( float(choice_line['baz']) )
                    
                    X, Y = get_instance_for_BAZReg_training(dataset_name=self.name,
                    ...
                    baz = float(self.csv_file['back_azimuth_deg'].iloc[choice_id])
                    X, Y = get_instance_for_BAZReg_training(dataset_name=self.name,
                                                            dataset_path = self.hdf5_path,
                                                            length_before_P = self.length_before_P,
                                                            length_after_P = self.length_after_P,
                    ...
                    baz = choice_line['path_backazimuth_deg']
                    X, Y = get_instance_for_BAZReg_training(dataset_name=self.name,
                                                            dataset_path = self.hdf5_path,
                                                            length_before_P = self.length_before_P,
                                                            length_after_P = self.length_after_P,
...
def get_Azi_training_dataset(cfgs):
    ...
    training_dataset_list = [tf.data.Dataset.from_generator(DiTingAziGenerator(training_dict_list[idx]),output_types = (tf.float32, tf.float32), output_shapes = ( (training_dict_list[idx]['length_before_P'] + training_dict_list[idx]['length_after_P'],training_dict_list[idx]['n_channels']), 2)) for idx in range(len(training_dict_list))]
    ...
    if len(validation_dict_list) >= 1:
        validation_dataset_list = [tf.data.Dataset.from_generator(DiTingAziGenerator(validation_dict_list[idx]),output_types = (tf.float32, tf.float32), output_shapes = ( (validation_dict_list[idx]['length_before_P'] + validation_dict_list[idx]['length_after_P'],validation_dict_list[idx]['n_channels']), 2)).repeat() for idx in range(len(validation_dict_list))]
